...
✅ Weekend 4 — README, metrics, limitations, HF Spaces deploy files. Not yet pushed to a live Space (needs a Hugging Face account).

## Live demo

_Not deployed yet — run locally with `python app.py` (Gradio) or `uvicorn app.api.main:app` (FastAPI)._
...
|---|---|---|---|
| My U-Net | 0.776 | 0.670 | 3.27 |
| Cellpose (pretrained, generalist) | 0.783 | 0.687 | 2.47 |

Evaluated head-to-head on 15 held-out test images with [train/eval.py](./train/eval.py) (sample size kept small — Cellpose's SAM backbone is slow on CPU). Cellpose still wins, as expected for a generalist foundation model vs. 8 epochs of training on 300 images — but the U-Net gets close (Dice within 0.007), which is a solid result for a from-scratch model on a small budget. W&B run: see `train/wandb/` (offline runs; sync with `wandb sync`).
...
- Densely overlapping or touching cells are the main failure mode for both models; the U-Net's watershed-based instance separation is more sensitive to under/over-segmentation on crowded fields than Cellpose's learned flow fields.
